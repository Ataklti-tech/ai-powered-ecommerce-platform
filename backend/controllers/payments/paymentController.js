...
exports.startPayment = catchAsync(async (req, res) => {
  ...
  switch (gateway) {
    ...
    case 'mtn_momo':
      ...
      break;
    case 'airtel':
      paymentData = await airtelService.startPayment(order, customer);
      break;
    case 'pesapal':
      paymentData = await pesapalService.startPayment(order, customer);
      break;
    default:
      return res.status(400).json({
        status: 'error',
        message: 'Invalid payment gateway. Use: stripe, paypal, mtn_momo, airtel, pesapal',
      });
  }
...
exports.verifyPayment = catchAsync(async (req, res) => {
  ...
  switch (gateway || order.payment.gateway) {
    ...
    case 'mtn_momo':
      ...
      break;
    case 'airtel':
      verificationResult = await airtelService.verifyPayment(transactionId);
      break;
    case 'pesapal':
      verificationResult = await pesapalService.verifyPayment(transactionId);
      break;
    default:
      return res.status(400).json({
        status: 'error',
